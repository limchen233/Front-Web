...
<script setup>
	...
	function setposition() {
		...
		latitude.value = state.currentLatitude;
		longitude.value = state.currentLongitude;
	...
	function regionchangetab(e) {
		...
		if (changel.value == 1) {
			if (e.type == 'end') {
				console.log('小程序拖动图标变化：',e.detail)
				page.value = 1;
				jump.value = 0;
	...
	function attachments() {
		myAmapFun.value.getRegeo({
			...
			page: page.value,
			success: function(data) {
				console.log('位置信息获取成功-map-plugin:',data);
				const {regeocodeData} = data[0]
				if (regeocodeData) {
					regeocodeData.pois.forEach((item, index) => {
						//成功回调
						list.value.push({
							id: item.id,
							name: item.name,
